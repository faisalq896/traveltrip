...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-goog-api-key': process.env.GEMINI_API_KEY }, body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }], generationConfig: { maxOutputTokens: 900, thinkingConfig: { thinkingLevel: 'minimal' }, responseMimeType: 'application/json' } }) });
    ...
    let result;
    try { result = JSON.parse(answer); } catch { result = { summary: answer.slice(0, 1200), suggestions: [], tips: [], warning: '' }; }
    return res.status(200).json({ summary: text(result.summary, 1200), suggestions: Array.isArray(result.suggestions) ? result.suggestions.slice(0, 4).map(item => ({ title: text(item?.title, 120), time: text(item?.time, 5), details: text(item?.details, 220), reason: text(item?.reason, 180) })) : [], tips: Array.isArray(result.tips) ? result.tips.slice(0, 4).map(item => text(item, 180)).filter(Boolean) : [], warning: text(result.warning, 240) });
  } catch {
